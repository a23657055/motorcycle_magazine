 ...
 * be shared and re-used across multiple threads.
 * </p>
 */
public class WebTOTPGenerator extends WebHOTPGenerator {
	private final long timeStepMillis;

	/**
	 * A string identifier for the HMAC-SHA1 algorithm (required by HOTP and
	 * allowed by TOTP). HMAC-SHA1 is the default algorithm for TOTP.
	 */
	 ...
	 */
	public WebTOTPGenerator() throws NoSuchAlgorithmException {
	 ...
	 *
	 * @param timeStep
	 *            the magnitude of the time-step for this generator
	 * @param timeStepUnit
	 *            the units for the the given time step
	 *
	 * @throws NoSuchAlgorithmException
	 *             if the underlying JRE doesn't support the
	 *             {@value tw.gov.ndc.ndcdashboard.web.WebHOTPGenerator#HOTP_HMAC_ALGORITHM}
	 ...
	 *             serious misconfiguration
	 */
	public WebTOTPGenerator(final long timeStep, final TimeUnit timeStepUnit)
			throws NoSuchAlgorithmException {
		this(timeStep, timeStepUnit, WebHOTPGenerator.DEFAULT_PASSWORD_LENGTH);
	}
	 ...
	 */
	public WebTOTPGenerator(final long timeStep, final TimeUnit timeStepUnit,
			final int passwordLength) throws NoSuchAlgorithmException {
		this(timeStep, timeStepUnit, passwordLength,
				WebHOTPGenerator.HOTP_HMAC_ALGORITHM);
	}
	 ...
	 *            generating passwords; TOTP allows for
	 *            {@value tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA1},
	 *            {@value tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA256},
	 *            and
	 *            {@value tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA512}
	 *
	 * @throws NoSuchAlgorithmException
	 *             if the underlying JRE doesn't support HMAC-SHA1, which should
	 *             never happen except in cases of serious misconfiguration
	 *
	 * @see tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA1
	 * @see tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA256
	 * @see tw.gov.ndc.ndcdashboard.web.WebTOTPGenerator#TOTP_ALGORITHM_HMAC_SHA512
	 */
	public WebTOTPGenerator(final long timeStep, final TimeUnit timeStepUnit,
			final int passwordLength, final String algorithm)
			throws NoSuchAlgorithmException {
	 ...
	 * @param timestamp
	 *            the timestamp for which to generate the password
	 *
	 ...
	 */
	public int generateOneTimePassword(final Key key, final Date timestamp)
			throws InvalidKeyException {
		return this.generateOneTimePassword(key,
				timestamp.getTime() / this.timeStepMillis);
	}
	 ...
	 *
	 * @param timeUnit
	 *            the units of time in which to return the time step
	 *
